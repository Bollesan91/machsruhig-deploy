...
- **Branch-Trick, gehärtet:** Artefakt committen, pushen, Reviewer bekommt **Commit-SHA-raw-URL** (`raw.githubusercontent.com/<user>/<repo>/<sha>/<pfad>`). **Nie Branch-URLs** — der Edge-Cache (~5 min) serviert sonst alte Stände (belegt: R3-Daten-PR). Bei Trust-/Matrix-Claims zusätzlich die Tool-Code-URLs unter demselben SHA mitgeben.
